...
class ContinentList(viewsets.ViewSet):
    ...
    def retrieve(self, request, continent_name=None, country_name=None, city_name=None):
        if country_name and city_name:
            queryset = Continent.objects.filter(continent_name=continent_name
                ).prefetch_related(Prefetch("country", queryset=Country.objects.filter(country_name=country_name
                ).prefetch_related(Prefetch('city', queryset=City.objects.filter(city_name=city_name))))
        )
        else:
